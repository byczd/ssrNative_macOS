...
struct s5_ctx {
    uint32_t arg0;  /* Scratch space for the state machine.状态机的暂存空间 */
...
uint8_t * s5_address_package_create(const struct s5_ctx *parser, void*(*allocator)(size_t size), size_t *size) {
    ...
    case s5_atyp_ipv4:  // IPv4
        memcpy(iter, parser->daddr, sizeof(struct in_addr)); //如果是ip地址,则返回[type(1)][ip(sizeof(in_addr))][port(2)]
    ...
    case s5_atyp_host:
        ...
        iter[0] = len;
        iter++;
        memcpy(iter, parser->daddr, len); //如果是domain，则返回[type(1)][addr_len(1)][domain(n)][port(2)]
        iter += len;
        break;
    default:
        assert(0);
