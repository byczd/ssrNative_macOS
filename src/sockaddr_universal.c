...
bool socks5_address_parse(const uint8_t *data, size_t len, struct socks5_address *addr) {
    ...
    addr_type = data[offset++];//01=ipv4，03=domain 04=ipv6
    ...
    switch ((enum SOCKS5_ADDRTYPE)addr_type) {
        ...
        case SOCKS5_ADDRTYPE_DOMAINNAME:
            ...
            memcpy(addr->addr.domainname, data+offset, addr_size);//addr->addr.domainname即为要访问的目标地址
            break;
    ...
    addr->port = ntohs( *((uint16_t *)(data+offset)) ); //其实也可以data[len-2-1,2]读取port
...
//
    int result = -1;
    if ((result = uv_inet_pton(AF_INET, addr_str, &addr->addr4.sin_addr)) == 0) { //pton,整形ip转点分十进制IP
        addr->addr4.sin_family = AF_INET;
