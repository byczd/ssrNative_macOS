...
struct socket_ctx {
    ...
    /* We only need one of these at a time so make them share memory. 我们一次只需要其中一个，所以让他们共享内存*/
...
struct tunnel_ctx {
    void* data;   //client_ctx* ctx
    bool is_terminated;
    uv_loop_t* loop; /* Backlink to owning loop object. 反向链接到拥有的循环对象。*/
    struct socket_ctx* incoming; /* Connection with the SOCKS client. 与SOCKS客户端的连接。*/
    struct socket_ctx* outgoing; /* Connection with upstream. 与上游的连接*/
    struct socks5_address* desired_addr; //目标信息：destAddr,destPort,destAddrType
    char extra_info[0x100];
    
    toto_session_outbound_t *outbound; //add.by.adong  for vmess&shadowsocks #include "vmessPro.h"
    
//    obfs_para_t *ss_obfs_para; //ss_obfs_unit.h
    ss_obfs_stage_t *ss_obfs; //add.by.adong for pro shadowsocks.tls  #include "ss_obfs_tls_pro.h"
    
    bool is_tls_protocol; //add.by.adong
    bool is_ws_protocol;  //add.by.adong
