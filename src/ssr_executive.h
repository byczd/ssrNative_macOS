...
// -------------ssr节点配置数据(表明server方)
    ...
    char *over_tls_server_domain; //实际用作ws-host
    char *over_tls_path;//实际用作ws-path 这2项不为空说明需要走ws协议
    
    char *proxytype; //add.by.adong
    bool over_tls_verify; //add.by.adong 是否需要tls校验，一般为no
// ---------------(表明client方)
    char *listen_host;
    unsigned short listen_port;
// -----------------
    struct cstl_map *user_id_auth_key; //身份认证key
    unsigned int max_client;  //最大连接数
    char *over_tls_root_cert_file; //add.by.adong, tls root证书文件(完整路径的cert文件，系统自动获取)
    bool udp; //是否支持udp,默认为true
    unsigned int idle_timeout; /* 连接空闲超时(毫秒) Connection idle timeout in ms. */
    uint64_t connect_timeout_ms; //连接超时(毫秒)
    uint64_t udp_timeout;//udp超时
    char *remarks; //标记
};

struct server_env_t {
    void *data;  //ssr_client_state *
