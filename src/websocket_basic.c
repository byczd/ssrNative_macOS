...
#define WEBSOCKET_REQUEST_FORMAT1                                               \
    "GET %s HTTP/1.1\r\n"                                                       \
    "Host: %s:%d\r\n"                                                           \
    "Upgrade: websocket\r\n"                                                    \
    "Connection: Upgrade\r\n"                                                   \
...
//    evbuffer_add_printf(out, "Origin:https://%s:%d\r\n", server_address,server_port); //missing this key will lead to 403 response.
//    evbuffer_add_printf(out, "\r\n");
//}

uint8_t * websocket_connect_request_Ex(const char *ws_host,const char *ws_path,const char *server_host, uint16_t server_port,
    const char *key, void*(*allocator)(size_t), size_t *result_len)
{
    uint8_t *buf = NULL;
    const char *fmt = WEBSOCKET_REQUEST_FORMAT1;
    size_t buf_len = 0;
    if (ws_host==NULL || server_host==0 || key==NULL || allocator==NULL) {
        return NULL;
    }
    ws_path = ws_path?ws_path:"/";

    buf_len = strlen(fmt) + strlen(ws_path) + 5 + strlen(ws_host) + strlen(key)+strlen(server_host)+5;
    ...
    sprintf((char *)buf, fmt, ws_path, ws_host, (int)server_port, key,server_host,(int)server_port);
...
#define WS_MASK_SIZE 4
    ...
    finNopcode = (info->fin ? 0x80 : 0x00) | info->opcode; //;  0x80|0x02 = 0x82;
    if(payload_len <= 125) {
...
uint8_t * websocket_build_frame_with_mask0(ws_frame_info *info, const uint8_t *payload, size_t payload_len, void*(*allocator)(size_t)) {
    uint8_t finNopcode;
    size_t payload_len_small;
    size_t payload_offset;
    size_t len_size = 0;
...
//    if (true) { //info->masking
        *(data + 1) = ((uint8_t)payload_len_small) | 0x80; // payload length with mask bit on
//    }
//    else {
//        *(data + 1) = ((uint8_t)payload_len_small) & 0x7F; //客户发送给服务端，必须要有mask，不可能走这里
//    }
    if(payload_len_small == 126) {
        payload_len &= 0xffff;
        *((uint16_t *)(data + 2)) = (uint16_t)ws_hton16((uint16_t)payload_len); //附加数据长度字节，为big大端字节顺序
    }
    if(payload_len_small == 127) {
        payload_len &= 0xffffffffffffffffLL;
        *((uint64_t *)(data + 2)) = (uint64_t)ws_hton64((uint64_t)payload_len);
    ...
    if (true) { //info->masking
        size_t i;
        uint8_t mask[WS_MASK_SIZE] = { 0xc2, 0xc3, 0xc4, 0xc5 };
        memcpy(data + (payload_offset - WS_MASK_SIZE), mask, WS_MASK_SIZE);
    ...
    info->frame_size = frame_size; //整个frame的长度
    info->payload_size = payload_len; //实际数据长度
...
uint8_t * websocket_retrieve_payload(const uint8_t *data, size_t dataLen, void*(*allocator)(size_t), ws_frame_info *info)
{
    ...
    char Opcode = 0;
    size_t count = 0;
    size_t len_bit = 0; //长度字段位数字
    size_t real_playload_len=0; //长度字段扩展位指示的实际数据长度
    ...
    do {
        ...
        if ((data[1] & 0x80) == 0x80) {
            flagMask = true; //服务端响应的数据一般不用mask，故flagMask=false
            count = 4;
        }

        len_bit = (size_t)(data[1] & 0x7F);
        if (len_bit == 126) {
            if(dataLen < 4) { return NULL; }
            real_playload_len = (size_t) ws_ntoh16( *((uint16_t *)(data + 2)) );
            ...
            real_playload_len = (size_t) ws_ntoh64( *((uint64_t *)(data + 2)) );
        ...
        if (dataLen < (packageHeadLen + real_playload_len)) {
            break; //一般是len=126时，所指的长度，超过了当前数据总长
        }
      
        package = (uint8_t *) allocator( real_playload_len + 1 ); //实际数据
        memset(package, 0, real_playload_len + 1); //此处会崩溃？

        if (flagMask) { //服务端返回数据，一般不用mask
            // 解包数据使用掩码时, 使用异或解码, maskKey[4] 依次和数据异或运算 .
            size_t i;
            for (i = 0; i < real_playload_len; i++) {
                uint8_t mask = maskKey[i % 4]; // maskKey[4] 循环使用 .
                package[i] = data[i + packageHeadLen] ^ mask;
            }
        } else {
            // 解包数据没使用掩码, 直接复制数据段...
            memcpy(package, data + packageHeadLen, real_playload_len);
        }
    ...
    if (info) {
        ...
        info->frame_size = real_playload_len + packageHeadLen;
        info->payload_size = real_playload_len;
