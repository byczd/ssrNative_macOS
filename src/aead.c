...
aead_decrypt(struct aead_buffer_t *ciphertext, struct aead_cipher_ctx_t *cipher_ctx, size_t capacity)
{
    ...
    brealloc(cipher_ctx->chunk, cipher_ctx->chunk->len + ciphertext->len, capacity);
    memcpy(cipher_ctx->chunk->data + cipher_ctx->chunk->len,ciphertext->data, ciphertext->len);
    ...
    if (!cipher_ctx->init) {
        ...
        memcpy(cipher_ctx->salt, cipher_ctx->chunk->data, salt_len); //首次则从data头部取出salt(即iv)

        if (ppbloom_check((void *)cipher_ctx->salt, salt_len) == 1) {
            LOGE("%s", "crypto: AEAD: repeat salt detected"); //检测到重复盐
            return CRYPTO_ERROR;
        }
