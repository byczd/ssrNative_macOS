...
// dump_level入参，Debug 级别，默认为 0 没有调试日志
    ...
    _init_ssl(mbed, host_name, dump_level); //mbed相关参数进行初始化
...
//---remote_addr为域名，则需要进一步dns解析
    req = (uv_getaddrinfo_t *)calloc(1, sizeof(*req));
    req->data = mbed;
    sprintf(portstr, "%d", port);
    status = uv_getaddrinfo(loop, req, _uv_dns_resolve_done_cb, remote_addr, portstr, NULL); //sg1.qiangdong.xyz:152
    if (status != 0) {
        free(req);
...
static void _init_ssl(uv_mbed_t *mbed, const char *host_name, int dump_level) {
    ...
    /* OPTIONAL is not optimal for security, but makes interop easier in this stage
     OPTIONAL并不是安全方面的最佳选择，但在这一阶段使互操作更加容易
     如果设置为MBEDTLS_SSL_VERIFY_OPTIONAL模式，那么客户端不会验证服务器证书的有效性
    */
    mbedtls_ssl_conf_authmode(&mbed->conf, MBEDTLS_SSL_VERIFY_OPTIONAL); //MBEDTLS_SSL_VERIFY_REQUIRED
    ...
    mbedtls_ssl_set_hostname(&mbed->ssl, host_name);//注意，这里设置的 hostname 必须对应服务器证书中的 common name，即 CN 字段。
//    if( ( ret = mbedtls_ssl_set_hostname( &ssl, "mbed TLS Server 1" ) ) != 0 )
    mbed->ssl_in = bio_new(true);
    mbed->ssl_out = bio_new(false);
    
    mbedtls_ssl_set_bio(&mbed->ssl, mbed, _mbed_ssl_send, _mbed_ssl_recv, NULL); //SSL context需要知道它需要用来发送网络流量的输入和输出功能.
}
...
//  client中为result = uv_tcp_connect(&socket->req.connect,&socket->handle.tcp, &socket->addr.addr,uv_socket_connect_done_cb);
        if (status < 0) {
...
#define UV_MBED_RCV_LEN (64 * 1024)
    ...
    do {
        struct bio *in = mbed->ssl_in;
        ssize_t recv = 0;
        uv_buf_t buf = uv_buf_init(NULL, 0); //这里buf置空，
        ...
        mbed->alloc_cb(mbed, UV_MBED_RCV_LEN, &buf); //这里buf仍旧置空
        ...
        mbed->read_cb(mbed, recv, &buf, mbed->read_cb_p); //buf只有初始，而后续没有赋值的机会，哪来的值？
        to_continue = (recv >= 0);
    } while(0);
    return to_continue;
}
