...
typedef enum
{
    ...
    MBEDTLS_SSL_SERVER_CERTIFICATE, //3服务器证书
    MBEDTLS_SSL_SERVER_KEY_EXCHANGE,//4服务器密钥交换
    MBEDTLS_SSL_CERTIFICATE_REQUEST,//5证书申请
    MBEDTLS_SSL_SERVER_HELLO_DONE, //6服务器HELLO完成
    MBEDTLS_SSL_CLIENT_CERTIFICATE, //7客户端证书
    MBEDTLS_SSL_CLIENT_KEY_EXCHANGE, //8客户端密钥交换
    MBEDTLS_SSL_CERTIFICATE_VERIFY, //9证书验证
    MBEDTLS_SSL_CLIENT_CHANGE_CIPHER_SPEC, //10客户端更改密码规范
    MBEDTLS_SSL_CLIENT_FINISHED,     //11 客户端已完成
    MBEDTLS_SSL_SERVER_CHANGE_CIPHER_SPEC, //12服务器更改密码SPE
    MBEDTLS_SSL_SERVER_FINISHED,    //13 服务器已完成
    MBEDTLS_SSL_FLUSH_BUFFERS,      //14 刷新缓冲区
    MBEDTLS_SSL_HANDSHAKE_WRAPUP,   //15握手包装
    MBEDTLS_SSL_HANDSHAKE_OVER,     //16 握手结束
    MBEDTLS_SSL_SERVER_NEW_SESSION_TICKET, //17 服务器新会话票证
    MBEDTLS_SSL_SERVER_HELLO_VERIFY_REQUEST_SENT, //18 已发送服务器HELLO验证请求
}
mbedtls_ssl_states;
 ...
 *              加载合理的默认SSL配置值。（首先需要调用mbedtls_ssl_config_iniit
 * \param conf     SSL configuration context
